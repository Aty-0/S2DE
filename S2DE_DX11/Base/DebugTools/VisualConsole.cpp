...
namespace S2DE
{
	VisualConsole::VisualConsole() : m_scroll_to_bottom(false), m_draw(false),
		m_free(false)
	{ 
	...
	void VisualConsole::Render()
	{
		...
		ImGui::Begin("Console", nullptr,
			m_free == false ? ImGuiWindowFlags_::ImGuiWindowFlags_NoMove | ImGuiWindowFlags_::ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_::ImGuiWindowFlags_NoTitleBar
		...
		case VisualConsoleSizeMode::Default:
			ImGui::SetWindowSize(ImVec2(float(Engine::GetGameWindow()->GetWidth() - 10), 400.0f));
				break;
		case VisualConsoleSizeMode::Fullscreen:
			ImGui::SetWindowSize(ImVec2(float(Engine::GetGameWindow()->GetWidth() - 10), float(Engine::GetGameWindow()->GetHeight() - 30)));
				break;
		case VisualConsoleSizeMode::Half:
			ImGui::SetWindowSize(ImVec2(float(Engine::GetGameWindow()->GetWidth() / 2), float(Engine::GetGameWindow()->GetHeight() - 30)));
				break;
		}
